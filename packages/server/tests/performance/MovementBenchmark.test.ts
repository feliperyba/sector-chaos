...
describe('Movement Performance', () => {
  it('processes 4-player movement step under 0.3ms average', () => {
    const playerCount = 4;
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (i % 10 === 0) enqueueMoveInputs(simulation, playerCount);
    }
    ...
    expect(avgStepMs).toBeLessThan(0.3);
  });

  it('handles 10-player movement under 0.5ms average', () => {
